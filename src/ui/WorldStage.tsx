...
export function WorldStage({room,night=false,restored=false}:{room:StageRoom;night?:boolean;restored?:boolean}){
  return <div className={`stage-art stage-${room} ${night?"stage-night":""} ${restored?"stage-restored":""}`} aria-hidden="true">
    ...
    <div className="prop-layer"><i className="forge-fire"/><i className="anvil"/><i className="weapon-rack"/><i className="barrels"/><i className="statue statue-a"/><i className="statue statue-b"/><i className="fountain"/><i className="gate-art"/><i className="sealed-door"/><i className="treasure-chest"/></div>
